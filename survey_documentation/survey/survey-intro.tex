...
and designers haven't had the need to think about the look and design
...
as friendly as possible, so that the user doesn't lose much time navigating around.
...
Media queries are very important because with them we can specify 
with what device we are accessing the web page. The query usually is made of
...
\begin{lstlisting}[%
    ...
    % An example of a media query, where we say that for a screen of size 850px or less, the background is black:
...
From this example above, we specified with the parameter \emph{screen}
the media type and using \emph{max-width} we determined the condition which
affects the device of the user. Now, we have a lot of different conditions which
we can use, and they are really helpful, because with them we can adjust our 
display settings for every device there is. 

For example, if the user is entering our website with his phone, we can automatically say that an image should have a lower resolution,
thus adjusting it better for his screen and saving some bandwith.

Another example would be using \emph{display:none} with which we can completely hide some content that we do not want
to be displayed.
...
%
If we would run this now in our browser, we wouldn't see the header, because
we decided not to show it. If we changed \emph{display:none} to \emph{display:block}
...
responsive web design comes in play[5]. With it, we can use percentage-based element sizing which is easier than using pixels. 

According by (Abdulrehman,2015) a flexible grid-layout is one of the foundations of responsive
design. Using the term "grid" doesn't mean necessarily that a grid framework needs to be used.
...
we can either use the same thing mentioned in the previous section, relative units,
if the screen gets too small then hide the image completely or by cropping some parts
of the image, again in the case if the screen gets too small[6].

\subsection{Relative measurements} 

Instead of using pixels, we use relative dimensions, percentage based. So instead of
...
Also, another option that one can use is cropping, which crops the picture to a certain width
...
In this Figure above, we can obviously see the result of tweaking the image a bit.
With just a few CSS commands, we can crop the picture and if we are browsing the
web site with a smaller device, the image doesn't have to be hidden, just its' size is changed.
