...
following code snippet we see how additional data in an HTML table can
be implemented. \parencite{Alligator}
...
The amount of data increases every second, over 2.5 quintillion bytes of
 ...
 world\parencite{PG_2}.
\newline
Tables are one of the best mediums for displaying large sets of data
...
DataTables is used\parencite{PG_1}.
...
Table is initilaized with "myTable" id and this id is used in ready
 ...
 instance\parencite{PG}.

An example HTML table with this technique when using the sort feature on
the \texttt{Price} column is shown below.
\begin{figure}[tp]
    \centering
